...
Yiora 移动端综合兴趣社区 App 的 Flutter 工程，当前为 **M2 页面层齐套 + M3 主体落地**：
登录注册（含找回密码）→ 首页（Banner/置顶条/推荐流 + 应用 Tab 软件库 + 全局搜索 + 话题聚合页）→ 圈子（圈内流最新/最热双 Tab）→ 发布（动态/软件双通道，软件含提取码；我的发布审核状态可查）→ 帖子详情（两级评论 + 举报）→ 软件详情（评论区/下载风险弹窗/版本记录）→ 任务中心（签到/领奖）→ 忧珠资产（余额/流水）→ 消息/私信（WS）→ 个人/他人主页 → 设置，全链路 Mock 可跑通。
...
flutter test        # 单测 + Widget 测试（当前 90 个全过）
...
└── features/                 # 按业务特性分包：data(API/仓库) / model / controller / view / widget
    ...
    ├── software/             # 应用中心/软件库：列表（类型/分类/排序筛选）+ 详情（截图横滑/版本记录/发布者/两级评论区）+ 下载（风险弹窗/链接提取码复制）（M3）
    ├── search/               # 全局搜索：帖子/用户/圈子/话题/软件五类 + 防抖 + 分页 + 结果跳转（M3）
    ├── topic/                # 话题聚合页：渐变头卡 + 最热/最新 + 帖子流（卡片/详情/搜索的话题 chip 均可进入）（M3）
    ├── growth/               # 成长激励：任务中心（每日/新手任务 + 签到阶梯奖励 + 领奖）+ 忧珠资产（余额 + 类型筛选流水）（M3）
...
- 软件库/搜索/举报/话题/成长客户端已按 server `types.go` 契约实现：
  ...
  `GET /software/mine`（审核状态）、`GET /search?type=&kw=&page=&size=`、`POST /reports`（targetType 1帖/2评/3用户）、
  `GET /circles/:id/posts?sort=new|hot`、`POST /auth/reset-password`、`GET /topics/:id/posts?sort=hot|new`、
  `GET /comments?bizType=2&bizId=` 与 `POST /comments`（软件评论）、`GET /tasks`、`POST /tasks/sign-in`、
  `POST /tasks/:id/claim`、`GET /youzhu/account`、`GET /youzhu/logs?bizType=`
...
- M3 余量：软件库 Banner 推荐位与运营合集、软件详情楼层回复全量展开（帖子详情已有，软件评论仅预览）
- M4：装扮商城、靓号商城、积分抽奖、兑换记录、付费帖解锁、AI 管家升级
